...
#roles de la aplicación de usuario grupo usugr
ROL_APP_USUGR = [
    (0, 'Usuario grupo', 'rol_usugr'),
    (1, 'Director de grupo', 'rol_dirusugr'),
    (2, 'Integrante de grupo', 'rol_intusugr'),
    (3, 'Director de proyeto de grupo', 'rol_dirpryusugr'),
    (4, 'Invitado', 'rol_invusugr'),
    (5, 'Anonimo', 'rol_anmusugr') #falto este
...
class usugr(models.Model): #ojo falta listado de productos y protyectos vinculados al grupo
    ...
    id_usu_admin = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, null= True, blank=True) #Identificador del usuario administrador (debe estar registrado y se le asignan permisos de administración de app_reg_gr)
...
class rel_usugr_ls_usu(models.Model):#Listado de id_usu integrantes del grupo registrados en el sistema.
    ...
    id = models.AutoField(primary_key = True)
    id_gr = models.ForeignKey(usugr, on_delete=models.CASCADE, null=False, blank =False)
    ls_int_usu = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, null= True, blank=True)
...
#class rl_usugr_pry(models.Model):  # listado de id procesos de formación realizados
#    id_usugr = models.ForeignKey(usugr_inf_gr, on_delete=models.CASCADE, null=False, blank =False)
#    prod = models.ForeignKey(inf_prog_inv, on_delete=models.CASCADE, null=False, blank =False)

#class rl_usugr_prod(models.Model):  # listado de id procesos de formación realizados
#    id_usugr = models.ForeignKey(usugr_inf_gr, on_delete=models.CASCADE, null=False, blank =False)
#    pry_inv = models.ForeignKey(inf_pry, on_delete=models.CASCADE, null=False, blank =False)
