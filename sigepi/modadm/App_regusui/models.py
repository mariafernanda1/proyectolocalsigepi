...
#roles de la aplicación de usuario institución
ROL_APP_USUI = [
    (0, 'Usuario institución', 'rol_usui'),
    (1, 'Coordinador de Institución', 'rol_corusui'),
    (2, 'Grupo Institucional', 'rol_grusui'),
    (3, 'Gestor Institucional', 'rol_gestusui'),
    (4, 'Responsable Unidad académica', 'rol_acausui'),
    (5, 'Evaluador', 'rol_evausui'),
    (6, 'Evaluador Externo', 'rol_evaext_usui'),
    (7, 'Auxiliar Administativo', 'rol_auxadm_usui'),
    (8, 'Usuario persona Institucional', 'rol_per_usui')
    ]
...
class usui(models.Model):
    ...
    id_usu_admin = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, null= True, blank=True) #Identificador del usuario administrador (debe estar registrado y se le asignan permisos de administración de app_reg_ins)
...
class rl_usui_rol_actual(models.Model): #relacion qu me almacena el rol actual
    id = models.AutoField(primary_key = True)
    id_usui = models.ForeignKey(usui, on_delete=models.CASCADE, null=True, blank = True) #
    rol_sis = models.ForeignKey(rol, on_delete=models.CASCADE, null=True, blank = True)
...
class rl_usui_usu(models.Model): #Listado de id de usuarios vinculados a la entidad o Institución.
    id = models.AutoField(primary_key = True)
    id_usui = models.ForeignKey(usui, on_delete=models.CASCADE, null=False, blank=False) # identificador unico
    ls_usu = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, null= True, blank=True)
...
class rl_usui_prod(models.Model):  #relacion Listado de id de convocatorias de investigación vinculados a la entidad o Institución.
    id = models.AutoField(primary_key = True)
    id_usui = models.ForeignKey(usui, on_delete=models.CASCADE, null=False, blank=False) # identificador unico
    ls_prod_inv = models.ForeignKey(inf_prog_inv, on_delete=models.CASCADE, null=False, blank=False)

class rl_usui_pry(models.Model):  #relacion Listado de id de convocatorias de investigación vinculados a la entidad o Institución.
    id = models.AutoField(primary_key = True)
    id_usui = models.ForeignKey(usui, on_delete=models.CASCADE, null=False, blank=False) # identificador unico
    ls_pry_inv = models.ForeignKey(inf_pry, on_delete=models.CASCADE, null=False, blank=False)
