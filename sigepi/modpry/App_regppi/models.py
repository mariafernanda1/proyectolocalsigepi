...
class prod_inv(models.Model): #preguntar nombre de la tabla ojo tipo_pro_inv
    ...
    id_usu = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, null=True, blank =True) #Identificador único del usuario que realizó el producto
    tipo =  models.ForeignKey(tipo_prod_inv, on_delete=models.CASCADE, null=False, blank =False)#, null=False, blank=False) # tipo de producto de investigación, ver Diccionario TIPO_PROD_INV
    ...
    id_proc_inv = models.ForeignKey(proc_inv, on_delete= models.CASCADE, null=False, blank=False) #Identificador del proceso de investigación del que es derivado.
...
class usu_inf_inv(models.Model):
    ...
    id_usu = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, null=True, blank =True)  # Identificador único de usuario # identificador unico
